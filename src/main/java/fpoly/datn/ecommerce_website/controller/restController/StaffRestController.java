...
public class StaffRestController {
    ...
    public ResponseEntity<List<StaffDTO1>> getAll() {
        ...
        return new ResponseEntity<>(
                list.stream().map(staff -> modelMapper.map(staff, StaffDTO1.class)).collect(Collectors.toList())
                , HttpStatus.OK
        );
    }
    ...
    public ResponseEntity<StaffDTO1> getOne(@RequestParam("staffId") String id) {
        return new ResponseEntity<>(
                modelMapper.map(staffService.findById(id), StaffDTO1.class)
                , HttpStatus.OK
        );
